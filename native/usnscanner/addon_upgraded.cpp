...
struct RecoveryStats {
    ...
    ULONGLONG recoveredBytes;
    ULONGLONG badSectorCount;
    ULONGLONG skippedBytes;
...
// Enhanced sector reading with bad sector detection and recovery
class EnhancedSectorReader {
private:
    HANDLE volumeHandle_;
    ULONGLONG clusterSize_;
    std::vector<ULONGLONG> badSectors_;
    DWORD maxRetries_;
    DWORD retryDelay_;

public:
    EnhancedSectorReader(HANDLE volumeHandle, ULONGLONG clusterSize) 
        : volumeHandle_(volumeHandle), clusterSize_(clusterSize), maxRetries_(3), retryDelay_(10) {}

    struct ReadResult {
        bool success;
        DWORD bytesRead;
        bool partialRead;
        std::vector<ULONGLONG> badSectorOffsets;
    };

    ReadResult ReadSectorWithRetry(LONGLONG offset, BYTE* buffer, DWORD size) {
        ReadResult result = {false, 0, false, {}};
        
        for (DWORD attempt = 0; attempt < maxRetries_; attempt++) {
            LARGE_INTEGER distance;
            distance.QuadPart = offset;
            
            if (!SetFilePointerEx(volumeHandle_, distance, nullptr, FILE_BEGIN)) {
            ...
            DWORD bytesRead = 0;
            if (ReadFile(volumeHandle_, buffer, size, &bytesRead, nullptr)) {
                result.success = true;
                result.bytesRead = bytesRead;
                result.partialRead = (bytesRead < size);
                return result;
            }
            ...
            if (error == ERROR_SECTOR_NOT_FOUND || 
                error == ERROR_CRC || 
                error == ERROR_IO_DEVICE ||
                error == ERROR_DISK_CORRUPT) {
                
                // Try to read sector by sector to isolate bad sectors
                if (size > 512 && attempt == maxRetries_ - 1) {
                    return ReadWithBadSectorDetection(offset, buffer, size);
                }
            }

            if (attempt < maxRetries_ - 1) {
                Sleep(retryDelay_ * (attempt + 1)); // Exponential backoff
            }
        }
    ...
    ReadResult ReadWithBadSectorDetection(LONGLONG baseOffset, BYTE* buffer, DWORD totalSize) {
        ReadResult result = {false, 0, true, {}};
        const DWORD sectorSize = 512;
        DWORD sectorsToRead = (totalSize + sectorSize - 1) / sectorSize;
        
        ZeroMemory(buffer, totalSize);
        
        for (DWORD sector = 0; sector < sectorsToRead; sector++) {
            LONGLONG sectorOffset = baseOffset + (sector * sectorSize);
            DWORD bytesToRead = min(sectorSize, totalSize - (sector * sectorSize));
            
            LARGE_INTEGER distance;
            distance.QuadPart = sectorOffset;
            
            if (!SetFilePointerEx(volumeHandle_, distance, nullptr, FILE_BEGIN)) {
                result.badSectorOffsets.push_back(sectorOffset);
                continue;
            }

            DWORD bytesRead = 0;
            if (ReadFile(volumeHandle_, buffer + (sector * sectorSize), bytesToRead, &bytesRead, nullptr)) {
                result.bytesRead += bytesRead;
                result.success = true;
            } else {
                // Mark this sector as bad and fill with zeros
                result.badSectorOffsets.push_back(sectorOffset);
                badSectors_.push_back(sectorOffset);
                ZeroMemory(buffer + (sector * sectorSize), bytesToRead);
            }
        }
    ...
    const std::vector<ULONGLONG>& GetBadSectors() const {
        return badSectors_;
    }

    void ClearBadSectorList() {
        badSectors_.clear();
    }
};
...
hanced Data Run Recovery Worker with advanced sector recovery
class DataRunRecoveryWorker : public Napi::AsyncWorker {
  public:
    ...
    void Execute() override {
        ...
        if (volumeHandle == INVALID_HANDLE_VALUE) {
            // Try without NO_BUFFERING flag for compatibility
        ...
        // Initialize enhanced sector reader
        EnhancedSectorReader reader(volumeHandle, clusterSize_);
        
        // Use adaptive buffer size based on available memory
        const size_t maxBufferSize = 64 * 1024 * 1024; // 64MB max
        const size_t minBufferSize = 64 * 1024;        // 64KB min
        size_t bufferSize = min(maxBufferSize, max(minBufferSize, static_cast<size_t>(clusterSize_ * 128)));
        
        std::vector<BYTE> buffer(bufferSize);
        std::vector<BYTE> zeroBuffer(bufferSize, 0);
        ...
        try {
            for (const auto &run : runs_) {
                ...
                ULONGLONG bytesToProcess = min(runBytesTotal, fileSize_ - totalProcessed);

                if (run.sparse || run.lcn <= 0) {
                    // Handle sparse runs (write zeros)
                    if (!WriteSparseData(outHandle, bytesToProcess, zeroBuffer)) {
                        hasErrors = true;
                        break;
                    }
                    stats_.skippedBytes += bytesToProcess;
                } else {
                    // Handle real data runs with enhanced recovery
                    LONGLONG absoluteOffset = static_cast<LONGLONG>(run.lcn) * static_cast<LONGLONG>(clusterSize_);
                    
                    if (!RecoverDataRun(reader, outHandle, absoluteOffset, bytesToProcess, buffer)) {
                        hasErrors = true;
                        stats_.partialRecovery = true;
                        // Continue with next run instead of failing completely
                    }
                }

                totalProcessed += bytesToProcess;
                
                // Update progress periodically
                double progress = (static_cast<double>(totalProcessed) / fileSize_) * 100.0;
                // Note: In a real implementation, you'd send progress updates to the main thread
            }

            // Ensure file is exactly the right size
            if (!hasErrors && totalProcessed < fileSize_) {
                ULONGLONG remaining = fileSize_ - totalProcessed;
                WriteSparseData(outHandle, remaining, zeroBuffer);
                stats_.skippedBytes += remaining;
                stats_.partialRecovery = true;
            }
            ...
            ::CloseHandle(volumeHandle);
            SetError("Unexpected error during recovery");
            return;
        }

        // Get final bad sector statistics
        const auto& badSectors = reader.GetBadSectors();
        stats_.badSectorCount = badSectors.size();

        ::CloseHandle(outHandle);
        ::CloseHandle(volumeHandle);

        // Set final recovery statistics
        if (stats_.recoveredBytes == 0 && stats_.badSectorCount > 0) {
            SetError("Recovery failed: too many bad sectors");
            return;
        }

        if (stats_.badSectorCount > 0) {
            stats_.partialRecovery = true;
        }
    }

private:
    bool WriteSparseData(HANDLE outHandle, ULONGLONG bytesToWrite, const std::vector<BYTE>& zeroBuffer) {
        ULONGLONG written = 0;
        while (written < bytesToWrite) {
            ULONGLONG chunk = min(bytesToWrite - written, static_cast<ULONGLONG>(zeroBuffer.size()));
            DWORD bytesWritten = 0;
            
            if (!::WriteFile(outHandle, zeroBuffer.data(), static_cast<DWORD>(chunk), &bytesWritten, nullptr)) {
                return false;
            }
    ...
    bool RecoverDataRun(EnhancedSectorReader& reader, HANDLE outHandle, 
                       LONGLONG startOffset, ULONGLONG totalBytes, 
        ...
        while (processed < totalBytes) {
            ULONGLONG chunk = min(totalBytes - processed, static_cast<ULONGLONG>(buffer.size()));
            LONGLONG currentOffset = startOffset + processed;

            auto result = reader.ReadSectorWithRetry(currentOffset, buffer.data(), static_cast<DWORD>(chunk));
            
            if (result.success) {
                // Write successfully read data
                DWORD bytesWritten = 0;
                if (!::WriteFile(outHandle, buffer.data(), result.bytesRead, &bytesWritten, nullptr)) {
                    return false;
                }
                
                stats_.recoveredBytes += result.bytesRead;
                
                // If partial read, fill remainder with zeros
                if (result.partialRead && result.bytesRead < chunk) {
                    ULONGLONG zeroBytes = chunk - result.bytesRead;
                    std::vector<BYTE> zeros(static_cast<size_t>(zeroBytes), 0);
                    
                    DWORD zeroWritten = 0;
                    if (!::WriteFile(outHandle, zeros.data(), static_cast<DWORD>(zeroBytes), &zeroWritten, nullptr)) {
                        return false;
                    }
                    
                    stats_.skippedBytes += zeroBytes;
                    success = false; // Mark as partial recovery
                }
            } else {
                // Complete read failure - write zeros and continue
                std::vector<BYTE> zeros(static_cast<size_t>(chunk), 0);
                DWORD zeroWritten = 0;
                
                if (!::WriteFile(outHandle, zeros.data(), static_cast<DWORD>(chunk), &zeroWritten, nullptr)) {
                    return false;
                }
                
                stats_.skippedBytes += chunk;
                success = false;
            }

            // Record bad sectors for reporting
            for (ULONGLONG badOffset : result.badSectorOffsets) {
                badSectorOffsets_.push_back(badOffset);
            }
...
public:
    void OnOK() override {
        ...
        result.Set("outputPath", Napi::String::New(env, WideToUtf8(outputPath_)));
        
        // Recovery statistics
        Napi::Object statsObj = Napi::Object::New(env);
        statsObj.Set("totalBytes", Napi::String::New(env, std::to_string(stats_.totalBytes)));
        statsObj.Set("recoveredBytes", Napi::String::New(env, std::to_string(stats_.recoveredBytes)));
        statsObj.Set("skippedBytes", Napi::String::New(env, std::to_string(stats_.skippedBytes)));
        statsObj.Set("badSectorCount", Napi::String::New(env, std::to_string(stats_.badSectorCount)));
        statsObj.Set("partialRecovery", Napi::Boolean::New(env, stats_.partialRecovery));
        
        double recoveryPercentage = stats_.totalBytes > 0 ? 
            (static_cast<double>(stats_.recoveredBytes) / stats_.totalBytes) * 100.0 : 0.0;
        statsObj.Set("recoveryPercentage", Napi::Number::New(env, recoveryPercentage));
        
        result.Set("stats", statsObj);

        // Bad sector information
        if (!badSectorOffsets_.empty()) {
            Napi::Array badSectors = Napi::Array::New(env, badSectorOffsets_.size());
            for (size_t i = 0; i < badSectorOffsets_.size(); ++i) {
                badSectors.Set(i, Napi::String::New(env, std::to_string(badSectorOffsets_[i])));
            }
            result.Set("badSectors", badSectors);
        }
    ...
    void OnError(const Napi::Error &e) override {
        ...
        errorResult.Set("success", Napi::Boolean::New(env, false));
        errorResult.Set("error", e.Value());
        
        // Include partial statistics even on error
        if (stats_.recoveredBytes > 0) {
            Napi::Object statsObj = Napi::Object::New(env);
            statsObj.Set("recoveredBytes", Napi::String::New(env, std::to_string(stats_.recoveredBytes)));
            statsObj.Set("badSectorCount", Napi::String::New(env, std::to_string(stats_.badSectorCount)));
            errorResult.Set("partialStats", statsObj);
        }
        
        Callback().Call({ errorResult, env.Undefined() });
    }
  ...
  private:
    ...
    std::vector<ULONGLONG> badSectorOffsets_;
};

// Helper function to parse runs array from JavaScript
bool ParseRunsArray(Napi::Env env, Napi::Array array, std::vector<DataRunSegment> &out, std::string &error) {
